...
CREATE PROCEDURE [dbo].[Workstations.Patching.Required.Last]
AS
     BEGIN
	--replace all CM_XXX with your SCCM DB name
         DELETE FROM [grafana].[dbo].[Workstations]
                WHERE [Filter] = 'Workstations.Patching.Required.Last';
		 	
         DECLARE @UserSIDs VARCHAR(16);
         SELECT @UserSIDs = 'disabled';
    -- workstations collection ID with updates deployed as required
		 DECLARE @CollID VARCHAR(8)= 'XXX0011A'; 
	...
	-- Adjust this Software Update Group Name filter to fit your Naming Convention
         ...
         INSERT INTO @SUG
                SELECT TOP 1 [CI_ID]
                       FROM [CM_XXX].[dbo].[v_AuthListInfo]
                       WHERE([title] LIKE '%workst%req%month%'
...
--select * from [v_StateNames]
         ...
         INSERT INTO [grafana].[dbo].[Workstations]
                SELECT DISTINCT
                       [t].[Name],
                       [t].[ResourceID],
                       [t].[StateName],
                       [t].[MachineStatus_Flag] AS [Compliance],
                       'Workstations.Patching.Required.Last' AS [Filter],
                       (
         ...
         INSERT INTO [grafana].[dbo].[Workstations]
                SELECT DISTINCT
                       ...
                       FROM [grafana].[dbo].[Workstations] [se]
